...
CLASSPATH=/system/framework/android.test.runner.jar:${base}/bin/uiautomator.jar
...
if [ "${cmd}" == "runtest" ]; then
  # logger.adb_auto_logger.debug deprecation warning
  ...
  while [ true ]; do
    ...
    if [ ! -f ${jar} ]; then
      ...
      # force to logger.adb_auto_logger.debug help message
      cmd="help"
      break
    fi
    jars=${jars}:${jar}
